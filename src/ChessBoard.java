public class ChessBoard {
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            if (!nowPlayer.equals(this.board[startLine][startColumn].getColor())) return false;
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                if (board[startLine][startColumn].getSymbol().equals("K")){
                    if (new King(board[startLine][startColumn].getColor()).isUnderAttack(this,endLine,endColumn)) {
                        System.out.println("Король окажется под атакой!");
                        return false;
                    }else {
                    ...
                    board[endLine][endColumn] = board[startLine][startColumn]; // if piece can move, we moved a piece
                    board[startLine][startColumn] = null; // set null to previous cell
                    if (positionKingAttack(this, nowPlayer)) {
    ...
    public boolean positionKingAttack(ChessBoard chessBoard, String playerTurn) {
        int kingLine = -1, kingColumn = -1;

        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (chessBoard.board[i][j] == null) continue;
                if (chessBoard.board[i][j].getSymbol().equals("K")) {
                    if (!chessBoard.board[i][j].getColor().equals(playerTurn)) {
                        kingLine = i;
                        kingColumn = j;
                    }
                }
        ...
        return new King(chessBoard.board[kingLine][kingColumn].getColor()).isUnderAttack(chessBoard, kingLine, kingColumn);
    }
